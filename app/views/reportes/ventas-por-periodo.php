<div class="page-header">
    <div>
        <h1><i class="fas fa-file-invoice-dollar"></i> Ventas por Período</h1>
        <p>Análisis de facturación</p>
    </div>
...
<div class="card">
    <div class="card-body">
        <form method="GET" action="<?php echo url('reportes/ventas-por-periodo'); ?>" class="rpt-filtros-form">
...
<div class="rpt-stats-grid">
    <div class="rpt-stat-card">
        ...
        <div class="rpt-stat-info">
            <span class="rpt-stat-label">Total Facturas</span>
            <span class="rpt-stat-value"><?php echo $totales['total_facturas'] ?? 0; ?></span>
        </div>
    </div>
    ...
    <div class="rpt-stat-card">
        ...
        <div class="rpt-stat-info">
            <span class="rpt-stat-label">Promedio por Factura</span>
            <span class="rpt-stat-value">Bs <?php echo number_format($totales['promedio_factura'] ?? 0, 2); ?></span>
        </div>
    </div>
    <div class="rpt-stat-card">
        <div class="rpt-stat-icon rpt-icon-max"><i class="fas fa-arrow-up"></i></div>
        <div class="rpt-stat-info">
            <span class="rpt-stat-label">Factura Máxima</span>
            <span class="rpt-stat-value">Bs <?php echo number_format($totales['factura_maxima'] ?? 0, 2); ?></span>
        </div>
    </div>
</div>
        ...
        <div class="rpt-grafico-container">
            ...
            <div class="rpt-grafico-barras">
                ...
                <div class="rpt-barra-grupo">
                    ...
                    <div class="rpt-barra" style="height: <?php echo max($porcentaje, 5); ?>%"></div>
...
<div class="card">
    <div class="card-header">
        <h2><i class="fas fa-list"></i> Facturas del Período</h2>
    </div>
    <div class="card-body">
        <?php if (empty($ventas)): ?>
        ...
        <?php else: ?>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo e($venta['cliente_nombres'] . ' ' . $venta['cliente_apellidos']); ?>
                                <br><small class="text-muted">CI: <?php echo e($venta['cliente_ci']); ?></small>
                            </td>
                            <td class="text-center"><?php echo $venta['cantidad_productos']; ?></td>
                            <td>
                                <?php
                                $estadoClases = [
                                    'PENDIENTE'    => 'badge-warning',
                                    'PAGO_PARCIAL' => 'badge-info',
                                    'PAGADA'       => 'badge-success',
                                    'VENCIDA'      => 'badge-danger'
                                ];
                                ?>
                                <span class="badge <?php echo $estadoClases[$venta['estado']] ?? 'badge-secondary'; ?>">
                                    <?php echo $venta['estado']; ?>
                                </span>
                            </td>
                            <td class="rpt-valor-debe"><strong>Bs <?php echo number_format($venta['total'], 2); ?></strong></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="pagination-wrapper">
                <div class="pagination-info">
                    Mostrando <?php echo (($page - 1) * $perPage) + 1; ?> - <?php echo min($page * $perPage, $total); ?> de <?php echo $total; ?> facturas
                </div>
                <?php $qBase = 'fecha_desde=' . urlencode($fecha_desde) . '&fecha_hasta=' . urlencode($fecha_hasta); ?>
                <div class="pagination">
                    <?php if ($page > 1): ?>
                        <a href="<?php echo url('reportes/ventas-por-periodo?' . $qBase . '&page=' . ($page - 1)); ?>" class="pagination-link"><i class="fas fa-chevron-left"></i> Anterior</a>
                    <?php endif; ?>
                    <?php
                    $startPage = max(1, $page - 2);
                    $endPage   = min($totalPages, $page + 2);
                    if ($startPage > 1): ?>
                        <a href="<?php echo url('reportes/ventas-por-periodo?' . $qBase . '&page=1'); ?>" class="pagination-link">1</a>
                        <?php if ($startPage > 2): ?><span class="pagination-dots">...</span><?php endif; ?>
                    <?php endif; ?>
                    <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                        <a href="<?php echo url('reportes/ventas-por-periodo?' . $qBase . '&page=' . $i); ?>" class="pagination-link <?php echo $i === $page ? 'active' : ''; ?>"><?php echo $i; ?></a>
                    <?php endfor; ?>
                    <?php if ($endPage < $totalPages): ?>
                        <?php if ($endPage < $totalPages - 1): ?><span class="pagination-dots">...</span><?php endif; ?>
                        <a href="<?php echo url('reportes/ventas-por-periodo?' . $qBase . '&page=' . $totalPages); ?>" class="pagination-link"><?php echo $totalPages; ?></a>
                    <?php endif; ?>
                    <?php if ($page < $totalPages): ?>
                        <a href="<?php echo url('reportes/ventas-por-periodo?' . $qBase . '&page=' . ($page + 1)); ?>" class="pagination-link">Siguiente <i class="fas fa-chevron-right"></i></a>
                    <?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
